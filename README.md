...
반도체 Sort/Final Test 공정에서 발생하는 ATE(Automated Test Equipment) 로그를 자동으로 분석하고, 이상 탐지 → 상태 분류 → 판정 → 정책 검증 → SPC 분석 → 운영자 리포트까지 수행하는 검증 플랫폼입니다.
...
> **참고:** OpenAI API key가 없으면 LLM report는 fallback으로 생성됩니다. 나머지 파이프라인은 정상 동작합니다.
